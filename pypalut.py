...
def color_distance(c1,c2):
    rm = 0.5*(c1[0]+c2[0])
    #d = sum( (2+rm,4,3-rm)*( (c1[0]-c2[0])+(c1[1]-c2[1])+(c1[2]-c2[2]))**2 )**0.5
...
'''def check_against_colors(c):
    smallest_distance = 1000.0
    nearest_match = -1
    count = 0
    for oc in colors_all:
        distance = color_distance(c,oc)
        if distance<smallest_distance:
            smallest_distance = distance
            nearest_match = count
        count += 1
    return [smallest_distance, nearest_match]'''
    ...
    if my_im != "error":
        ...
        for x in range(0,w-1):
            for y in range(0,h-1):
                c = l[x,y]
                #print c
                #c = (c[0]/255.0,c[1]/255.0,c[2]/255.0) #convert to 0-1
                ...
                count = 0
                for tc in table_hls:
                    distance = color_distance(chls,tc)
                    #print distance
